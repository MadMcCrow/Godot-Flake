...
let
  ...
  # build dependencies
  mkBuildDependencies = options:
    (with pkgs; [ zlib yasm ]) ++ conditionalLib (use_x11 options) libXorg
...
in {
  ...
  mkRuntimeDependencies = options:
    libRuntime ++ conditionalLib (use_x11 options) libXorg
    ++ conditionalLib (use_mono options) libMono
    ++ conditionalLib (use_openGL options) libOpenGL
    ++ conditionalLib (use_vulkan options) libVulkan;
  ...
  mkNativeBuildInputs = options:
    (mkBuildTools options) ++ (mkBuildDependencies options);
